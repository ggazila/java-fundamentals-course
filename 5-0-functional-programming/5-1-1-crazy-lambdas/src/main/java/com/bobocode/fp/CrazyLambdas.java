...
public class CrazyLambdas {
    ...
    public static Function<BigDecimal, String> toDollarStringFunction() {
        return number -> String.format("$%s", number);
    }

    /**
     * Receives two parameters that represent a range and returns a {@link Predicate<String>} that verifies if string
    ...
    public static IntUnaryOperator boundedRandomIntSupplier() {
        return (randomInt) -> ThreadLocalRandom.current().nextInt(randomInt);
    }
    ...
    public static BiFunction<IntUnaryOperator, IntPredicate, IntUnaryOperator> functionToConditionalFunction() {
        return (intUnaryOperator, intPredicate) -> operand -> {
            boolean test = intPredicate.test(operand);
            if (test) {
    ...
    public static BiFunction<Map<String, IntUnaryOperator>, String, IntUnaryOperator> functionLoader() {
        return (map, str) -> operand -> {
            boolean b = map.containsKey(str);
            if (b) {
                IntUnaryOperator intUnaryOperator = map.get(str);
                int i = intUnaryOperator.applyAsInt(operand);
                return i;
            }
            IntUnaryOperator identity = IntUnaryOperator.identity();
            return identity.applyAsInt(operand);
        };
    }
