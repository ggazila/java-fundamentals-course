...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream().anyMatch(baseEntity -> baseEntity.getUuid() == null);
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            return getT(elements, comparator);
        }

        private static <T> Optional<T> getT(Iterable<T> elements, Comparator<? super T> tComparator) {
            ...
            for (T element : elements) {
                if (tComparator.compare(max, element) < 0) {
                    max = element;
                }
            }
         ...
         */
        public static void swap(List<? super Object> elements, int i, int j) {
            Objects.checkIndex(i, elements.size());
            Objects.checkIndex(j, elements.size());
            Object iObject = elements.get(i);
            Object jObject = elements.get(j);
            elements.set(i, jObject);
            elements.set(j, iObject);
        }
